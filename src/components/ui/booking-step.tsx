...
type BookingStep = "slot" | "notes" | "confirm";
const AVATAR_COLORS = [
...
// Public entry point. Just decides *whether* a booking modal is open and
// *for whom* — all of the step/slot/notes machinery lives in BookingModal
// below, which is remounted fresh (via `key`) every time a different
// practitioner is selected. That's the React-recommended way to reset a
// component's internal state on prop change — no effect, no synchronous
// setState-in-effect cascade.
export function BookingFlow({ selected, onClose }: BookingFlowProps) {
  return (
    <AnimatePresence>
      {selected && (
        <BookingModal
...
function BookingModal({ practitioner, onClose }: BookingModalProps) {
  // Defaults to true — bookable every day — unless this practitioner has
  // explicitly opted out of weekend consultations.
  const worksWeekends = practitioner.worksWeekends ?? true;
  ...
  const [bookingStep, setBookingStep] = useState<BookingStep>("slot");
  ...
  } = useConsultantAvailability(practitioner.id, dateISO);
  ...
  async function confirmBooking() {
    ...
    try {
      ...
      // return (see the useEffect in consultations-page.tsx) or via
      // webhook.
  ...
  return (
    ...
    >
      <motion.div
        ...
        className="bg-[#1A2030] border border-white/10 rounded-2xl w-full max-w-lg shadow-2xl max-h-[calc(100vh-2rem)] flex flex-col overflow-hidden"
      >
        {/* Modal header */}
        <div className="flex items-center gap-3 p-5 border-b border-white/[0.07] shrink-0">
          <div
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[12px] text-white/40">
              {typeConfig[practitioner.type].label}
            </p>
          </div>
        ...
        <div className="p-5 overflow-y-auto">
            ...
            <div>
              ...
              {bookingError && (
                <div className="flex items-start gap-2.5 p-3 rounded-xl bg-red-500/10 border border-red-500/20 mb-4">
                  <AlertTriangle
                    ...
                    className="text-red-400 shrink-0 mt-0.5"
                  />
                  <p className="text-[12px] text-red-300">{bookingError}</p>
                </div>
              )}
              ...
              <div className="flex items-center justify-between text-[13px] text-white/40 mb-5">
                ...
                <span className="font-semibold text-white">
                  ₦{CONSULTATION_PRICES[practitioner.type].toLocaleString()}
                </span>
              </div>
              <button
          ...
          {bookingStep === "notes" && (
            <div>
              <p className="text-[13px] text-white/50 mb-4">
                Let <span className="text-white">{practitioner.name}</span> know
            ...
            <div>
              ...
              <div className="space-y-3 mb-5">
                {[
                  ...
                  {
                    label: "Specialty",
                    value: typeConfig[practitioner.type].label,
                  },
                  {
                    label: "Date & Time",
                  ...
                  {
                    label: "Fee",
                    value: `₦${CONSULTATION_PRICES[practitioner.type].toLocaleString()}`,
                  },
                ].map((f) => (
                  <div
                    key={f.label}
                  ...
                  >
                    <span className="text-[12px] text-white/35">{f.label}</span>
                ...
                {bookingNotes && (
                  <div className="pt-1">
                    <p className="text-[12px] text-white/35 mb-1">Your Notes</p>
              ...
              <p className="text-[11px] text-white/25 text-center mt-3">
                ...
                Your session is only confirmed — and your meeting link generated
